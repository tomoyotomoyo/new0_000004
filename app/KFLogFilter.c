...
// === Externs for KRW backend (populated during init_xpf) ==================
extern uint64_t g_msgbufp_addr;
extern uint64_t g_msgbuf_size;
// Weak declarations so the filter still links/compiles if krw.m/XPF stubbed.
__attribute__((weak)) void kreadbuf(uint64_t kaddr, void *buf, size_t len) {
...
// === Backend 3: File polling (/var/log) ====================================
...
KFLogMode KFLogInitFilter(KFLogFilter *filter) {
    ...
    // Priority 3: FILE. If sandbox_escape() has been applied, we can walk
