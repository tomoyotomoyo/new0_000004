...
//
//  Three backends (selected at init, in priority order):
...
typedef enum {
    ...
    KFLOG_MODE_KRW    = 2,  // kread64 from kernel msgbuf; full exploit
...
typedef struct {
    ...
    uint64_t msgbuf_addr;   // (KRW mode) kernel msgbuf virtual address
    uint64_t msgbuf_size;   // (KRW mode) size of ring buffer
    uint64_t read_offset;   // (KRW mode) last read position
